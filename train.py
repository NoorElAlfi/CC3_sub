...
def run_training(name, team, name_of_agent, max_eps, write_to_file=False):
    ...
    if technique == 'DDQN':
        ...
        agents = {f"blue_agent_{agent}": PPO(wrapped_cyborg.observation_space(f"blue_agent_{agent}").shape[0], len(wrapped_cyborg.get_action_space(f'blue_agent_{agent}')), 0.002, [0.9, 0.990], 0.99, 4, 0.2) for agent in range(18)}
        print(f'Using agents {agents}, if this is incorrect please update the code to load in your agent')
        #if write_to_file:
                ...
                for agent_name, agent in agents.items():
                    if agent_name in actions:
                        if agent_name in wrapped_cyborg.agents:
                            ...
                            agent.memory.rewards.append(-10)
...
if __name__ == "__main__":
    ...
    #technique = 'DDQN'
    #run_training(name, team, technique, 100)
    technique = 'PPO'
    run_training(name, team, technique, 100)
