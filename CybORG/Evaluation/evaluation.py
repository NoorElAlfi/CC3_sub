...
def run_evaluation(name, team, name_of_agent, max_eps, write_to_file=True):
    ...
    agents = {f"blue_agent_{agent}": PPO(wrapped_cyborg.observation_space(f"blue_agent_{agent}").shape[0], len(wrapped_cyborg.get_action_space(
        f'blue_agent_{agent}')), 0.002, [0.9, 0.990], 0.99, 4, 0.2, True, '/cage/CybORG/Evaluation/submission/Models/5110.pth') for agent in range(18)}
    print(
    ...
    for i in range(max_eps):
        ...
        count = 0
        for j in range(500):
            actions = {agent_name: agent.get_action(observations[agent_name], agent.memory, deterministic=True)
                       for agent_name, agent in agents.items() if agent_name in wrapped_cyborg.agents}
