...
from emu_agents.rdqn.sum_tree import SumTree
        ...
        for key in self.buffer.keys():  # numpy->tensor
            if key == 'action':
                batch[key] = torch.tensor(
                    self.buffer[key][index], dtype=torch.long)
            else:
                batch[key] = torch.tensor(
                    self.buffer[key][index], dtype=torch.float32)
    ...
    def store_transition(self, state, action, reward, next_state, terminal, done):
        ...
        if len(self.n_steps_deque) == self.n_steps:
            ...
            self.current_size = min(
                self.current_size + 1, self.buffer_capacity)
...
class Prioritized_ReplayBuffer(object):
    ...
    def sample(self, total_steps):
        ...
        self.beta = self.beta_init + \
            (1 - self.beta_init) * (total_steps /
                                    self.max_train_steps)  # beta：beta_init->1.0
        batch = {}
        for key in self.buffer.keys():  # numpy->tensor
            if key == 'action':
                batch[key] = torch.tensor(
                    self.buffer[key][batch_index], dtype=torch.long)
            else:
                batch[key] = torch.tensor(
                    self.buffer[key][batch_index], dtype=torch.float32)

        return batch, batch_index, IS_weight

    # 根据传入的td_error，更新batch_index所对应数据的priorities
...
class N_Steps_Prioritized_ReplayBuffer(object):
    ...
    def store_transition(self, state, action, reward, next_state, terminal, done):
        ...
        if len(self.n_steps_deque) == self.n_steps:
            ...
            # 更新当前经验在sum_tree中的优先级
            self.sum_tree.update(data_index=self.count, priority=priority)
            # When 'count' reaches buffer_capacity, it will be reset to 0.
            self.count = (self.count + 1) % self.buffer_capacity
            self.current_size = min(
                self.current_size + 1, self.buffer_capacity)
    ...
    def get_n_steps_transition(self):
        ...
        # 获取deque中最后一个transition的s'和terminal
        next_state, terminal = self.n_steps_deque[-1][3:5]
        n_steps_reward = 0
