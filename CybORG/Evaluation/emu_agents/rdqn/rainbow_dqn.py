...
import numpy as np
import copy
from emu_agents.rdqn.network import Dueling_Net, Net
...
class DQN(object):
        ...
        with torch.no_grad():  # q_target has no gradient
            if self.use_double:  # Whether to use the 'double q-learning'
                ...
                a_argmax = self.net(batch['next_state']).argmax(
                    dim=-1, keepdim=True)  # shape：(batch_size,1)
                ...
                q_target = batch['reward'] + self.gamma * (1 - batch['terminal']) * self.target_net(
                    batch['next_state']).max(dim=-1)[0]  # shape：(batch_size,)

        # shape：(batch_size,)
        q_current = self.net(batch['state']).gather(-1,
                                                    batch['action']).squeeze(-1)
    ...
    def lr_decay(self, total_steps):
        lr_now = 0.9 * self.lr * \
            (1 - total_steps / self.max_train_steps) + 0.1 * self.lr
