...
class PPO_discrete:
    ...
    def update(self, replay_buffer, total_steps):
        ...
        for _ in range(self.K_epochs):
            ...
            for index in BatchSampler(SubsetRandomSampler(range(self.batch_size)), self.mini_batch_size, False):
                ...
                a_logprob_now = dist_now.log_prob(
                    a[index].squeeze()).view(-1, 1)  # shape(mini_batch_size X 1)
                # a/b=exp(log(a)-log(b))
    ...
    def save(self, file):
        self.actor.save(file)
        self.critic.save(file)

    def load(self, file):
        self.actor.load(file)
        self.critic.load(file)
