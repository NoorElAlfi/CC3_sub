...
from submission.Agent.DDQN import Agent;
import numpy as np
# this imports a submissions agents
#from submission.submission import agents, wrap


def wrap(env):
    return PettingZooParallelWrapper(env=env)

def run_evaluation(name, team, name_of_agent, max_eps, write_to_file=False):
    ...
    agents = {f"blue_agent_{agent}": Agent(wrapped_cyborg, observation, wrapped_cyborg.action_space(f'blue_agent_{agent}'), f'blue_agent_{agent}') for agent in range(18)}
    ...
    for i in range(max_eps):
        ...
        for i in agents:
            agents[i].memory.state.append(observations[i])
        ...
        count = 0
        for j in range(500):
            actions = {agent_name: agent.get_action(observations[agent_name]) for agent_name, agent in agents.items() if agent_name in wrapped_cyborg.agents}
            observations, rew, done, info = wrapped_cyborg.step(actions)
            #print(done)  
            for i in agents:
                agent_state = observations[i] if i in observations.keys() else None
                agent_action = actions[i] if i in actions.keys() else action_spaces[i].sample()
                agent_reward = rew[i] if i in rew.keys() else 0
                agent_done = done[i] if i in done.keys() else True
                if type(agent_state) != np.ndarray:
                    pass
                else:
                    agents[i].memory.update(agent_state, agent_action, agent_reward, agent_done)
            if all(done.values()):
                break
