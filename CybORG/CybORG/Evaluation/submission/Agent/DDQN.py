...
class Agent(BaseAgent):
    ...
    def build_network(self):
        self.Q_network = Model(self.obs_set_number,
                               self.action_number)  # .cuda()
        self.target_network = Model(
            self.obs_set_number, self.action_number)  # .cuda()
    ...
    def update_Q_network(self, state, action, reward, state_new, terminal):
        ...
        y = (reward + torch.mul(((self.target_network.forward(state_new)
             * action_new_onehot).sum(dim=1)*terminal), Config.discount_factor))
